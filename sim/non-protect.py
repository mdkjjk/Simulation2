...
class ProtectExample(LocalProtocol):
    ...
    def run(self):
        ...
        for i in range(self.num_runs):
            ...
            self.send_signal(Signals.SUCCESS, result_tel)

def sim_setup(node_a, node_b, num_runs):
    pro_example = ProtectExample(node_a, node_b, num_runs)
    ...
    def record_run(evexpr):
        ...
        return {"fidelity": f2, "time": result_tel["time"]}
...
def run_experiment_node(node_distances): # シミュレーションを実行するための関数
    ...
    for node_distance in node_distances:
        ns.sim_reset()
        network = network_setup(node_distance=node_distance) # ネットワーク構築
        node_a = network.get_node("node_A")
        node_b = network.get_node("node_B")
        example, dc = sim_setup(node_a, node_b, 1000) # シミュレーション設定
        example.start()
